...
This means you can use `ffigen` to generate native/FFI bindings, jsgen to generate for JS/WASM bindings.  If you use the `ffi-native` option when generating your native/FFI bindings, you will be able to use a single library file with a conditional import to switch between the two at compile time.

For example, take the following header function definition:
...
This will import the correct bindings file depending on the platform being targeted. With some exceptions (see below), if you use `ffi-native` to generate your FFI bindings and you always import the conditional import library file, you won't have to adjust your calling code depending on whether you're targeting WASM or native platforms.
